...
void visit_type_q_obj_x_nbd_server_add_bitmap_arg_members(Visitor *v, q_obj_x_nbd_server_add_bitmap_arg *obj, Error **errp)
{
    Error *err = NULL;
    ...
    if (visit_optional(v, "bitmap-export-name", &obj->has_bitmap_export_name)) {
        visit_type_str(v, "bitmap-export-name", &obj->bitmap_export_name, &err);
        if (err) {
            goto out;
        }
    }
