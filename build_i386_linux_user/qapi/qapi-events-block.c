...
void qapi_event_send_pr_manager_status_changed(const char *id, bool connected, Error **errp)
{
    QDict *qmp;
    Error *err = NULL;
    QMPEventFuncEmit emit;
    ...
    visit_start_struct(v, "PR_MANAGER_STATUS_CHANGED", NULL, 0, &err);
    if (err) {
        goto out;
    }
    visit_type_q_obj_PR_MANAGER_STATUS_CHANGED_arg_members(v, &param, &err);
    if (!err) {
        visit_check_struct(v, &err);
    }
    visit_end_struct(v, NULL);
    ...
    visit_complete(v, &obj);
    qdict_put_obj(qmp, "data", obj);
    emit(QAPI_EVENT_PR_MANAGER_STATUS_CHANGED, qmp, &err);
