...
void qmp_marshal_x_nbd_server_add_bitmap(QDict *args, QObject **ret, Error **errp)
{
    Error *err = NULL;
    Visitor *v;
    q_obj_x_nbd_server_add_bitmap_arg arg = {0};
    ...
    visit_type_q_obj_x_nbd_server_add_bitmap_arg_members(v, &arg, &err);
    if (!err) {
        visit_check_struct(v, &err);
    }
    visit_end_struct(v, NULL);
    ...
    qmp_x_nbd_server_add_bitmap(arg.name, arg.bitmap, arg.has_bitmap_export_name, arg.bitmap_export_name, &err);
...
out:
    ...
    v = qapi_dealloc_visitor_new();
    visit_start_struct(v, NULL, NULL, 0, NULL);
    visit_type_q_obj_x_nbd_server_add_bitmap_arg_members(v, &arg, NULL);
    visit_end_struct(v, NULL);
    visit_free(v);
}
